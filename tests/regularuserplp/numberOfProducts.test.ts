...
test.describe("Number of Products", async () => {

    test("Verify that the total number of filtered out products is displayed at the top right corner of the product grid section.", { tag: '@regression @noOfProducts @regularUserPlp ' }, async () => {
        ...
        await test.step("Step 11:verify total number of the products is visible", async () => {
            await PLPPage.verifyTotalCountOfProductsIsVisible(page);
        })
    })
})
